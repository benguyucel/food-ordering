...
export default withAuth(
    async function middleware(req: NextRequest) {
        ...
        if (isAdminRoute(pathname) && !includes(['admin'], token.role)) {
            return NextResponse.redirect(new URL('/', `${process.env.NEXT_API_BASE_URL}`))
        }
        if (pathname.startsWith('/api/users')) {
            let userId = pathname.split('/')[3]
            if (typeof userId === "undefined" && !includes(["admin"], token.role)) {
                return NextResponse.redirect(new URL('/', `${process.env.NEXT_API_BASE_URL}`))
            }
            if (userId !== token.uid || !includes(['admin', 'user'], token.role)) {
                return NextResponse.redirect(new URL('/', `${process.env.NEXT_API_BASE_URL}`))
            }
        }
    }
